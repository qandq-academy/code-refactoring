...
export function WithDataCalculator<P, T>(WrappedComponent: React.ComponentType<P & CalculationResult<T>>, func?: (state: DataContextModel)=> T) {
    const ComponentWithExtraInfo = (props: P & ExtraInfoType) => {
        return (
            <DataContext.Consumer>
              {(state: DataContextModel) => <WrappedComponent {...props} val={func ? func(state) : state.findValue(props.calculation!) as T} />}
            </DataContext.Consumer>
        );
    };
